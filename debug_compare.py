...
def clear_debug_files():
    ...
    print(f"Cleared existing debug files in {OUTPUT_DIR}")
...
def run_cpp_debug_inference(model_path: str, input_text: str = None) -> bool:
    ...
    try:
        ...
        sys.path.append(str(Path(__file__).parent / "InfiniCore-Infer-main" / "scripts"))
        from qwen3 import QwenForCausalLM
        from libinfinicore_infer import DeviceType
        
        # --- 修复: 使用正确的DeviceType枚举 ---
        model = QwenForCausalLM(model_path, device=DeviceType.DEVICE_TYPE_CPU, ndev=1)
        print("✓ Qwen3 model created successfully")
        # Enable debug mode via the C++ API

        # --- 修复: 移除不存在的调试函数 ---
        # 由于调试函数不存在，我们跳过调试模式设置
        print("⚠ Debug mode functions not available in this version")
        ...
        if input_text is None:
            ...
            # --- 修复: generate_simple 只返回字符串，不返回元组 ---
            output_text = model.generate_simple(input_text, max_steps=1)
...
def compare_debug_outputs() -> List[Dict]:
    ...
    py_files = list(OUTPUT_DIR.glob("py_*.txt"))
    py_files.sort()
    
    # Find all C++ debug files  
    cpp_files = list(OUTPUT_DIR.glob("cpp_*.txt"))
    cpp_files.sort()
    ...
    for py_file in py_files:
        ...
        # --- 修改: 使用OUTPUT_DIR ---
        cpp_file = OUTPUT_DIR / cpp_file_name
...
def main():
    ...
    if not args.skip_python:
        ...
        try:
            hidden_states = run_debug_inference(model_path, args.input, output_dir=OUTPUT_DIR)
