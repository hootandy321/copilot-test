 ...
 * 
 * 权重分区策略：
 * - 总注意力头数：nh（查询头）+ 2*nkvh（键 + 值头）
 * - 每设备头数：(nh + 2*nkvh) / ndev
 * - 每个设备独立处理其分配的头切片
 * - 连接的 QKV 格式：沿头维度的 [Q_heads, K_heads, V_heads]
 * 
 * 内存布局和偏移计算：
 * - 全局权重形状：[d, (nh + 2*nkvh) * dh] 或转置
 * - 每设备切片：[d, (nh + 2*nkvh)/ndev * dh]
 * - 字节偏移：idev * heads_per_device * dh * d * sizeof(data_type)
 * 
 * 张量属性：
 * - 输入维度：d（模型隐藏大小）
 * - 输出维度：(nh + 2*nkvh)/ndev * dh（每设备头数 * 头维度）
 * - 最终形状：[d, (nh + 2*nkvh)/ndev * dh] 用于矩阵乘法
 * - 分布：按注意力头在设备间分区
 * 
 * 矩阵运算：QKV = hidden_states @ weights_slice
 * - hidden_states：[batch_size, d]
 ...
 * - idev：当前设备索引（0 到 ndev-1）
 * - ndev：设备总数
 * 
 * 返回：设备特定的 QKV 投影权重 [(nh+2*nkvh)/ndev * dh, d] 或 [d, (nh+2*nkvh)/ndev * dh]
 */
inline std::shared_ptr<Tensor> getAttnQKV(
     ...
     * 计算设备切片的内存偏移
     * 
     * 每个设备获得 (nh + 2*nkvh)/ndev 个注意力头。
     * 偏移计算考虑：
     * - 设备索引：idev
     * - 每设备头数：(nh + 2*nkvh) / ndev
     * - 头维度：dh  
     * - 模型维度：d
     * - 数据类型大小：dsize(w->dt_mat)
     */
     ...
     * 
     * 权重可能以两种格式存储：
     * 1. 标准：[d, output_dim] - 准备用于矩阵乘法
     * 2. 转置：[output_dim, d] - 需要置换以正确使用
     */
    if (w->transpose_linear_weights != 0) {
        // 权重存储为 [(nh + 2*nkvh)/ndev * dh, d]，转置为 [d, (nh + 2*nkvh)/ndev * dh]
        auto shape = std::vector<size_t>({(nh + 2 * nkvh) / ndev * dh, d});
        return Tensor::weight((char *)(w->attn_qkv[layer]) + offset, w->dt_mat, shape)
            ->permute({1, 0});  // 转置到正确方向
    } else {
        // 权重已处于正确格式：[d, (nh + 2*nkvh)/ndev * dh]
...
/*
 * 为分布式注意力提取 QKV 偏置权重（可选）
 * 
 * 为 QKV 投影偏置项创建设备特定的张量包装器。
 * 这些偏置项是可选的，仅在某些模型配置中存在。
 * 与 QKV 权重一样，偏置项按注意力头在设备间分区。
 * 
 * 偏置分区：
 * - 总偏置元素：(nh + 2*nkvh) * dh  
 * - 每设备元素：(nh + 2*nkvh)/ndev * dh
 * - 每个设备获得其分配的注意力头的偏置项
 * 
 * 内存布局：
 * - 全局偏置形状：[(nh + 2*nkvh) * dh]
 * - 每设备切片：[(nh + 2*nkvh)/ndev * dh]
 * - 字节偏移：idev * heads_per_device * dh * sizeof(data_type)
 * 
 * 用法：在注意力计算之前添加到 QKV 投影输出：
 ...
 * - ndev：设备总数
 * 
 * 返回：设备特定的 QKV 偏置项 [(nh+2*nkvh)/ndev * dh]
 */
