...
def save_tensor_debug(tensor, name: str, output_dir: Path, step: Optional[int] = None):
    """Save tensor data for debugging comparison to a specific directory."""
    if step is not None:
...
class DebugQwen3MLP(torch.nn.Module):
    ...
    def forward(self, x, output_dir: Path, debug_layer: Optional[int] = None):
        # Gate and Up projections
        gate = self.gate_proj(x)
        up = self.up_proj(x)
        
        if debug_layer is not None:
            save_tensor_debug(gate, "mlp_gate_proj", output_dir, debug_layer)
            save_tensor_debug(up, "mlp_up_proj", output_dir, debug_layer)
        ...
        if debug_layer is not None:
            save_tensor_debug(activated_up, "mlp_activated_up", output_dir, debug_layer)
            save_tensor_debug(intermediate, "mlp_intermediate", output_dir, debug_layer)
        ...
        if debug_layer is not None:
            save_tensor_debug(output, "mlp_output", output_dir, debug_layer)
...
class DebugQwen3Attention(torch.nn.Module):
    ...
    def forward(
        ...
        if debug_layer is not None:
            save_tensor_debug(query_states, "attn_q_proj_raw", output_dir, debug_layer)
            save_tensor_debug(key_states, "attn_k_proj_raw", output_dir, debug_layer)
            save_tensor_debug(value_states, "attn_v_proj_raw", output_dir, debug_layer)
        ...
        if debug_layer is not None:
            save_tensor_debug(query_states, "attn_q_rope", output_dir, debug_layer)
            save_tensor_debug(key_states, "attn_k_rope", output_dir, debug_layer)
        ...
        if debug_layer is not None:
            save_tensor_debug(attn_weights, "attn_scores", output_dir, debug_layer)
        ...
        if debug_layer is not None:
            save_tensor_debug(attn_weights, "attn_weights", output_dir, debug_layer)
        ...
        if debug_layer is not None:
            save_tensor_debug(attn_output, "attn_values_output", output_dir, debug_layer)
        
        # Output projection
        attn_output = self.o_proj(attn_output)
        
        if debug_layer is not None:
            save_tensor_debug(attn_output, "attn_o_proj", output_dir, debug_layer)
...
class DebugQwen3DecoderLayer(torch.nn.Module):
    ...
    def forward(
        ...
        if debug_outputs:
            save_tensor_debug(hidden_states, "input_hidden_states", output_dir, self.layer_idx)
        
        # Pre-attention normalization
        hidden_states = self.input_layernorm(hidden_states)
        
        if debug_outputs:
            save_tensor_debug(hidden_states, "attn_norm_output", output_dir, self.layer_idx)
        ...
        if debug_outputs:
            save_tensor_debug(hidden_states, "attn_residual_output", output_dir, self.layer_idx)
        ...
        if debug_outputs:
            save_tensor_debug(hidden_states, "mlp_norm_output", output_dir,self.layer_idx)
        
        hidden_states = self.mlp(hidden_states, output_dir=output_dir, debug_layer=self.layer_idx if debug_outputs else None)
        hidden_states = residual + hidden_states
        
        if debug_outputs:
            save_tensor_debug(hidden_states, "layer_output", output_dir, self.layer_idx)
...
class DebugQwen3Model(torch.nn.Module):
    ...
    def forward(
        ...
        if debug_outputs:
            save_tensor_debug(input_ids, "input_ids", output_dir, "input")
            save_tensor_debug(hidden_states, "input_embeddings", output_dir, "input")
        ...
        if debug_outputs:
            save_tensor_debug(position_embeddings[0], "position_cos", output_dir, "input")
        ...
        if debug_outputs:
            save_tensor_debug(hidden_states, "final_norm_output", output_dir, "output")
        
        
        return hidden_states


def run_debug_inference(model_path: str, input_text: Optional[str] = None, output_dir: Optional[Path] = None) -> torch.Tensor:
    ...
    print("\nRunning Python debug inference...")
    with torch.no_grad():
        # --- 修改: 传递 output_dir ---
        hidden_states = model(input_ids, output_dir=output_dir, debug_outputs=True)
    
    print(f"Inference completed. Final hidden states shape: {hidden_states.shape}")
    print(f"Debug output files saved to '{output_dir}' with prefix 'py_'")
...
if __name__ == "__main__":
 ...
 # --- 修改: 创建一个默认的输出目录 ---
    output_dir = Path("./output_ref_only")
    run_debug_inference(model_path, input_text, output_dir=output_dir)