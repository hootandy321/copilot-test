...
inline std::shared_ptr<Tensor> getQwen3AttnQKV(
    Qwen3Meta const *meta,
    Qwen3Weights const *w,
...
inline std::shared_ptr<Tensor> getQwen3AttnQKVBias(
    Qwen3Meta const *meta,
    Qwen3Weights const *w,
    size_t layer, size_t idev, size_t ndev) {
    auto nkvh = meta->nkvh;
...
// Qwen3-specific: Q normalization weights
inline std::shared_ptr<Tensor> getQwen3QNorm(
    ...
    return Tensor::weight((char *)(w->q_norm[layer]), w->dt_norm, shape);
}
...
inline std::shared_ptr<Tensor> getQwen3KNorm(
    ...
    auto shape = std::vector<size_t>({meta->dh});
    return Tensor::weight((char *)(w->k_norm[layer]), w->dt_norm, shape);
}

inline std::shared_ptr<Tensor> getQwen3FFNNorm(
    Qwen3Meta const *meta,
    Qwen3Weights const *w,
    size_t layer) {
    auto shape = std::vector<size_t>({meta->d});
...
inline std::shared_ptr<Tensor> getQwen3FFNGateUp(
    Qwen3Meta const *meta,
    Qwen3Weights const *w,
    size_t layer, size_t idev, size_t ndev) {
    auto di = meta->di;
...
inline std::shared_ptr<Tensor> getQwen3FFNDown(
    Qwen3Meta const *meta,
    Qwen3Weights const *w,
    size_t layer, size_t idev, size_t ndev) {
    auto di = meta->di;
