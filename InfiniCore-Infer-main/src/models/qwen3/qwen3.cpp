...
void inferQwen3DeviceBatch(const Qwen3Meta &meta, Qwen3DeviceResource &rsrc,
    ...
    // Qwen3-specific: Q/K normalization descriptors
    std::vector<infiniopRMSNormDescriptor_t> desc_q_norm, desc_k_norm;
    if (has_qk_norm) {
        desc_q_norm.resize(nlayer);
        desc_k_norm.resize(nlayer);
        for (size_t layer = 0; layer < nlayer; layer++) {
            // Q normalization: reshape to (ntok * nh, dh) for per-head normalization
            auto q_reshaped = TensorDesc::create(dt_logits, {ntok * nh, dh});
            RUN_INFINI(infiniopCreateRMSNormDescriptor(
                rsrc.handle, &desc_q_norm[layer], q_reshaped->desc(), q_reshaped->desc(),
                rsrc.w_q_norm[layer]->desc(), meta.epsilon));
            RUN_INFINI(infiniopGetRMSNormWorkspaceSize(desc_q_norm[layer], &temp_size));
            workspace_size = std::max(workspace_size, temp_size);
            
            // K normalization: reshape to (ntok * nkvh, dh) for per-head normalization  
            auto k_reshaped = TensorDesc::create(dt_logits, {ntok * nkvh, dh});
            RUN_INFINI(infiniopCreateRMSNormDescriptor(
                rsrc.handle, &desc_k_norm[layer], k_reshaped->desc(), k_reshaped->desc(),
                rsrc.w_k_norm[layer]->desc(), meta.epsilon));
    ...
    if (has_qk_norm) {
        for (size_t layer = 0; layer < nlayer; layer++) {
            infiniopDestroyRMSNormDescriptor(desc_q_norm[layer]);
            infiniopDestroyRMSNormDescriptor(desc_k_norm[layer]);
        }
    }
...
// KV Cache functions (simplified implementation, reusing jiuge logic)
__C struct KVCache *
createQwen3KVCache(const struct Qwen3Model *model) {
    // TODO: Implement KV cache creation for Qwen3
...
duplicateQwen3KVCache(const struct Qwen3Model *model,
    ...
    // TODO: Implement KV cache duplication for Qwen3
    return nullptr;
}
